...
object task2 {
  ...
  def pcy(baskets: Iterator[Array[String]], basketSize: Long, bucketSize: Long, support: Int): Iterator[List[List[String]]] = {
    ...
    breakable {
      while (true) {
        ...
        val tempBitmap = new LargeBitmap(bucketSize.toInt)
        for ((k, v) <- hashTable) {
          if (v >= ps)
  ...
  def firstPass(basketsArr: Array[Array[String]], ps: Double, bucketSize: Long): (collection.mutable.Set[String], LargeBitmap) = {
    ...
    val bitmap = new LargeBitmap(bucketSize.toInt)
    for((k, v) <- hashTable) {
      if (v >= ps) {
  ...
  def getHashSum(stringVal: String): Long = {
    val unicode = Math.abs(stringVal.map(_.toByte).sum.toInt)
    unicode
  }
  ...
  def getHash(args: List[String], bucketSize: Long): Long = {
    var hashSum = 0L
    for (arg <- args) {
      hashSum += getHashSum(arg)
    }
    val hash = hashSum.hashCode() % bucketSize
    hash
  }

  def driver(threshold: Int, support: Int, inputFile: String, outputFile: String): Unit = {
    ...
    val rdd = lines.map(x => (x.split(",")(0), x.split(",")(1)))
      ...
      .filter(x => x._2.length >= threshold)
      .map(x => x._2).cache()
  ...
  def main(args: Array[String]): Unit = {
    val Array(threshold, support, inputFile, outputFile) = args

    time(driver(threshold.toInt, support.toInt, inputFile, outputFile))
  }
}
